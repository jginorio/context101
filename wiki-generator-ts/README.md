# Wiki Generator (TypeScript)

TypeScript wiki generator deployed by CDK to the wiki Fargate task
(`cdk/lib/context101-stack.ts` → `WikiGenImage` from `wiki-generator-ts/`).
The frozen Python tree under `wiki-generator/` has been removed; this directory
...
1:1 of the original Python generator — same corpus filtering, same no-change hash, same
...
`", "` separators) so structural files stay comparable across historical runs.
...
```
src/
  config.ts      env parsing (mirrors the original generate.py config block)
...
Env vars honored: `AWS_REGION`, `AWS_PROFILE`, `DOCS_BUCKET` (required),
`WIKI_PREFIX`, `WIKI_MODE`, `CORPUS_PREFIX`, `REPO_FULL_NAME`, `MODEL_ID`,
`MODEL_PROVIDER`, `LLM_KEY_SECRET_ARN`, `MIN_PAGES`, `MAX_PAGES`,
`CORPUS_PREVIEW_CHARS`, `CORPUS_SUMMARY_MAX_CHARS`, `MAX_TOKENS`, `WIKI_FORCE`,
`XML_DUMP_PATH`. This generator also adds `HARNESS_MODEL` (see below).
...
## Deterministic surfaces to validate
...
1. Point a local run at a separate prefix so it doesn't clobber live wiki pages:
   ```bash
   DOCS_BUCKET=my-bucket WIKI_PREFIX=wiki-preview/ WIKI_FORCE=1 npm run dev
   ```
2. Check structural outputs and behavior:
   - **`_meta.json`** — `corpus_sha` is a pure hash of the corpus inputs.
   - **`_index.json`** — stable page count; `slug`, `importance`, and `sources`
...
3. Then flip the flags on (`WIKI_AUTOLINK=1`, `WIKI_GAPS=1`) and eyeball the
   new behavior separately.

## Known non-equivalences vs the historical Python generator
...
Wired into CDK: `WikiGenImage` uses this directory's Dockerfile. Do not point
the DockerImageAsset back at a deleted `wiki-generator/` path.
