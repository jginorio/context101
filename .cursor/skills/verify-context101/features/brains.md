...
- **Empty stack (code-guaranteed).** Catalog `[]` on Knowledge (and wiki / sources / suggestions / conflicts) shows heading `No brains yet` and primary button `Create a brain` (not “Pick another brain”, not “registered under default”). That link is `/brains?new=1`, which opens the existing `Create a brain` dialog. `/brains` itself still shows `No brains yet` + `Create the first brain`. Do not submit create in a default run.
