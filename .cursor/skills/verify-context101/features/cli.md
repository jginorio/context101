...
- `cli-list` runs `context101 list [--aws-profile <name>]` with no checkout. Expect a Context101 deployments table (`NAME` / `STATUS` / `UPDATED`).
- `cli-destroy-dry-run` runs `context101 destroy <StackName> --dry-run [--aws-profile <name>]`. Must print `dry-run — destroy nothing` and `Would destroy <StackName>`. Must not destroy.
...
- `list`, `help`, and `destroy --dry-run` need no `cdk/` + `web/` checkout. `init` / `deploy` / `diff` / `synth` / destroy-for-real need a checkout or clone.
