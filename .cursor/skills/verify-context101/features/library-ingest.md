...
- `ingest-ask-ui` is skipped while wiki chrome is parked. Use `bin/retrieve` instead of `/wiki/ask`.

## How to get to it (user POV)

- Create, rename, or delete a Library file (see [library-files](./library-files.md)).
- Call `POST /api/wiki/retrieve` with `{ "message": "…" }` while signed in (`bin/retrieve`). Wiki chrome (`/wiki/ask`) is parked.
...
- **Ask UI.** Skip while wiki is parked (`/wiki/ask` redirects). Proof is `bin/retrieve`.
...
- Default filter excludes `source=code-wiki` and `source=wiki` (not `github`). Hits under `wiki/` and GitHub source-code files (`.ts`, `.py`, …) are dropped after retrieve. GitHub-synced `.md` / `.mdx` / `.txt` and manual uploads (no `source` sidecar) are included.
