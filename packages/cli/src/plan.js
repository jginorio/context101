...
export function formatDryRun(plan) {
  const lines = [
    ...
    `     BETTER_AUTH_URL / APP_URL left unset until the Amplify domain exists`,
...
export function nextSteps(plan) {
  return [
    ...
    "After Amplify is up (~4 min), open /setup on that domain and create the first admin.",
    "BETTER_AUTH_URL and APP_URL can be filled in the env file once you have the domain, then redeploy.",
  ].join("\n");
}
